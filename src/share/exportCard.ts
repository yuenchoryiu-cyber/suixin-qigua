...
async function renderClassic(data: ShareCardData): Promise<string> {
  ...
  const followLines = data.followUp?.trim()
    ...
    : []
  const followBlock = followLines.length ? 40 + followLines.length * 26 : 0
  const h = Math.max(960, 280 + Math.max(bodyLines.length, 1) * 28 + followBlock + 100)
...
async function renderCompact(data: ShareCardData): Promise<string> {
  ...
  const h = 280 + Math.max(lines.length, 1) * 24
  const scale = 2
  canvas.width = w * scale
  canvas.height = h * scale
  ctx = canvas.getContext('2d')!
...
async function renderPoster(data: ShareCardData): Promise<string> {
  ...
  const followLines = data.followUp?.trim()
    ? wrapByWidth(ctx, data.followUp.trim(), maxW)
    : []
  const followBlock = followLines.length ? 48 + followLines.length * 28 : 0
  const h = Math.max(1100, 420 + Math.max(bodyLines.length, 1) * 30 + followBlock + 80)
  const scale = 2
  canvas.width = w * scale
  canvas.height = h * scale
  ctx = canvas.getContext('2d')!
